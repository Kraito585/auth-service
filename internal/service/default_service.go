...
func (s *DefaultService) RegisterPartner(ctx context.Context, req model.SRequest2GerAPI) (string, error) {
	// 1. Нормализуем URL для CORS
	...
	payload := struct {
		...
		TokenHash:    hashedKey, // Добавляем наш хэш!
	}
